...
def save_output_to_kml(dataframe, metric, theme, file_name):
    ...
    gdf = gpd.GeoDataFrame(df, geometry='geometry')[[metric, 'No.Samples','geometry']]
    ...
    for idx, row in gdf.iterrows():
        ...
        attributes = row[[metric,'No.Samples']]  # Adjust the column names as per your data
        description = '<table>'
        table_html = """
            <style>
            table {
...
def generate_pilot_output(area_name, date_str, polygon_df, bin_size, df_raw, output_dir):
    ...
    df_rsrp = df_segment[["polygon_str", "Best Cell RSRP"]]
    df_rsrp_median = df_rsrp.groupby('polygon_str').median()
    df_rsrp_count = df_rsrp.groupby('polygon_str').count()
    ...
    grouped = df_segment.groupby(['polygon_str', 'Best Cell Label'])
    ...
    top_cell_list.sort()
    df4 = df_segment[['polygon_str','Best Cell Label','Best Cell RSRP']]
    df4 = df4[df4['Best Cell Label'].isin(top_cell_list)]
    df4_median = df4.groupby(['Best Cell Label','polygon_str']).median()
    df4_count = df4.groupby(['Best Cell Label','polygon_str']).count()
    df4 = df4_median
    ...
    for cell, subset in df4.groupby(level='Best Cell Label'):
        ...
        # display(subset)
        subset = subset[['polygon_str','Best Cell RSRP', 'No.Samples']]
    ...
    ##================================== EACH CELL ONE COLOR =========================
    df5 = aggregated.loc[aggregated.groupby('polygon_str')['rank'].idxmax()]
    ...
    for idx, row in gdf.iterrows():
        ...
        count = row['count']
        table_html += f'<tr><td>{cell_label}</td><td>{count}</td></tr>'
        table_html += '</table>'
